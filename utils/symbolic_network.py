...
class SymbolicLayer(nn.Module):
    ...
    def __init__(self, funcs=None, initial_weight=None, init_stddev=0.1, in_dim=None):
        ...
        if self.initial_weight is not None:     # use the given initial weight
            ...
            # Change to nn.Parameter so that Module.to(device) will move it together
...
class SymbolicNetL00(nn.Module):
    """Symbolic regression network with multiple layers. Produces one output."""
    def __init__(self, symbolic_depth, in_dim=1, funcs=None,
        ...
        layer_in_dim = [in_dim] + [len(funcs)] * self.depth

        # First build each SymbolicLayerL0
        if initial_weights is not None:
            layers = [
                SymbolicLayerL0(funcs=funcs,
                                initial_weight=initial_weights[i],
                                in_dim=layer_in_dim[i])
                for i in range(self.depth)
            ]
            # Do not set output_weight immediately, do it later
        else:
            if not isinstance(init_stddev, list):
                init_stddev = [init_stddev] * self.depth
            layers = [
                SymbolicLayerL0(funcs=funcs,
                                init_stddev=init_stddev[i],
                                in_dim=layer_in_dim[i])
                for i in range(self.depth)
            ]
            # Similarly delay initializing output_weight

        # —— Bind hidden_layers early ——
        self.hidden_layers = nn.Sequential(*layers)

        # Then initialize the linear weights of the last layer
        if initial_weights is not None:
            self.output_weight = nn.Parameter(initial_weights[-1].clone().detach())
        else:
            last_n = self.hidden_layers[-1].n_funcs
            self.output_weight = nn.Parameter(torch.rand((last_n, 1)) * 2)
